...
public interface StoryRepository extends Neo4jRepository<Story, String> {
    ...
    // ─────────────────────────────────────────────────────────────
    // AUTHOR RELATIONSHIP
    // (user_profile)-[:CREATED_STORY {createdAt}]->(story)
    // ─────────────────────────────────────────────────────────────

    /** Create the CREATED_STORY edge from the author's user_profile to the story. */
    @Query("""
    ...
    /** Find stories older than a given timestamp (for the scheduled 24h cleanup job). */
    @Query("""
            MATCH (s:story)
