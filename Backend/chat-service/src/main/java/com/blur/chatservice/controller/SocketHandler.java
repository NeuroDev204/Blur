...
import com.blur.chatservice.service.*;
...
public class SocketHandler {
  ...
  private void handleConnect(SocketIOClient client) {
    try {
      ...
      if (token == null || token.isEmpty()) {
        throw new BlurException(ErrorCode.TOKEN_REQUIRED);
      }
      ...
      if (!introspectRes.isValid()) {
        throw new BlurException(ErrorCode.INVALID_TOKEN);
      }
  ...
  private void handleDisconnect(SocketIOClient client) {
    ...
    try {
      ...
      throw new BlurException(ErrorCode.DISCONNECT_FAILED);
    }
  }
  ...
  private void handleSendMessage(SocketIOClient client, Map<String, Object> data, Object ack) {
    try {
      ...
      if (senderId == null) {
        throw new BlurException(ErrorCode.SESSION_EXPIRED);
      }
      ...
      if (conversationId == null || conversationId.isEmpty()) {
        throw new BlurException(ErrorCode.CONVERSATION_ID_REQUIRED);
      }
      ...
      var conversation = conversationRepository
          ...
          .orElseThrow(() -> new BlurException(ErrorCode.CONVERSATION_NOT_FOUND));

      if (conversation.getParticipants().size() != 2) {
        throw new BlurException(ErrorCode.INVALID_CONVERSATION);
      }
      ...
      String receiverId = conversation.getParticipants().stream()
          ...
          .findFirst()
          .orElseThrow(() -> new BlurException(ErrorCode.RECEIVER_NOT_FOUND));
  ...
  private void handleTyping(SocketIOClient client, Map<String, Object> data, Object ack) {
    try {
      ...
      throw new BlurException(ErrorCode.SEND_EVENT_FAILED);
    }
  }
  ...
  public void onCallInitiate(SocketIOClient client, Map<String, Object> data, Object ack) {
    ...
    try {
      ...
      if (callerId == null || receiverId == null || callTypeStr == null) {
        throw new BlurException(ErrorCode.INVALID_DATA);
      }
      ...
      try {
        ...
        throw new BlurException(ErrorCode.INVALID_CALL_TYPE);
      }
      ...
      if (receiverSocketId == null) {
        throw new BlurException(ErrorCode.USER_NOT_AVAILABLE);
      }
      ...
      if (sessionId != null) {
        try {
          ...
          throw new BlurException(ErrorCode.CALL_STATUS_UPDATE_FAILED);
        }
      }
  ...
  public void onCallAnswer(SocketIOClient client, Map<String, Object> data, Object ack) {
    try {
      ...
      if (session == null) {
        throw new BlurException(ErrorCode.CALL_NOT_FOUND);
      }
      ...
      new Thread(() -> {
        try {
          ...
          throw new BlurException(ErrorCode.THREAD_INTERRUPTED);
        }
      })
          .start();
  ...
  private void onWebRTCOffer(SocketIOClient client, Map<String, Object> data, AckRequest ack) {
    try {
      ...
      if (sent == 0) {
        throw new BlurException(ErrorCode.PEER_NOT_FOUND);
      }
